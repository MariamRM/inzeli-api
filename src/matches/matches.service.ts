...
export class MatchesService {
  ...
  async createMatch(input: {
    ...
    winners: string[];
    losers: string[];
    stakeUnits?: number; // ignored now
  }) {
    const { roomCode, sponsorCode, gameId } = input;
    const winners = (input.winners ?? []).filter(Boolean);
    const losers  = (input.losers  ?? []).filter(Boolean);
    ...
    const incByWinner = new Map<string, number>();
